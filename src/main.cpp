...
void ControlBoard() {  
  if (LcontrolBoard == false) {;  // ควบคุม Board ของตัวเอง
    addnumber();  //ใส่ตัวเลข
    ...
    if (Control_Board == "ควบคุม Plug ไฟ"){ 
      // if (results.value == ir_one){digitalWrite(Relay1,!digitalRead(Relay1));Blynk.virtualWrite(V1, !digitalRead(Relay1));}            
      // if (results.value == ir_two){digitalWrite(Relay2,!digitalRead(Relay2));Blynk.virtualWrite(V2, !digitalRead(Relay2));}
      // if (results.value==ir_three){digitalWrite(Relay3,!digitalRead(Relay3));Blynk.virtualWrite(V3, !digitalRead(Relay3));}
      // if (results.value== ir_four){digitalWrite(Relay4,!digitalRead(Relay4));Blynk.virtualWrite(V4, !digitalRead(Relay4));}
      // if (results.value== ir_five){digitalWrite(Relay1, LOW);digitalWrite(Relay2, LOW);digitalWrite(Relay3, LOW);digitalWrite(Relay4, LOW);Blynk.virtualWrite(V1,1);Blynk.virtualWrite(V2,1);Blynk.virtualWrite(V3,1);Blynk.virtualWrite(V4,1);}      
      // myData.z = 0;
    }else{  
