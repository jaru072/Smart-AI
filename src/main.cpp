...
BLYNK_WRITE(V0) { String CReadString = param.asString();
  if (!CReadString.isEmpty()) { 
    Serial.println(CReadString);
    // terminal.flush();
  }
}  
...
BLYNK_WRITE(V4) {NTerminal = param.asInt();}
...
BLYNK_WRITE(V7) { FilePlay = param.asInt();
  if (FilePlay != 0) { // ยกเลิกทักทายตอนเปิดเครื่อง
    Lwait_Slogan1 = true;Lwait_Slogan2 = true;Leof_speech = true;N = TotalASpeech+1;
    Leof_mp3 = 0;audio.connecttoSD(AFolderFile[FolderPlay][FilePlay].c_str());
  }  
}
...
BLYNK_CONNECTED() {
  ...
  Blynk.virtualWrite(V4, 1);Blynk.syncVirtual(V4);
...
void setup() {
  ...
  terminal.clear();
  terminal.flush();
  timer1.setInterval(20000L, myTimerEvent);
}
...
void loop() {
  ...
  if (millis() - last_timer > 2000) {last_timer = millis();
    if (Wifi_Connect == true){ GetTimeInternet();
      ...
      if (NTerminal == 1) {CTerminal = CDateTime_Thai+" "+CTime+CMoonPhaseThai;}
      if (NTerminal == 2) {CTerminal = CDateTime_Thai+" "+CTime+" s="+Leof_speech+" mp3="+Leof_mp3;}
      if (NTerminal == 3) {CTerminal = CDateTime_Thai+" "+CTime+CMoonPhaseThai;CTerminal = CDateTime_Thai+" "+CTime+" "+CTerminal.length();}
      Serial.println(C_Print); terminal.println(CTerminal.substring(0,82));
    }
